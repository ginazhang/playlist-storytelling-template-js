define(["storymaps/playlist/config/MapConfig", "esri/map",
  ...
  function (MapConfig,
    ...
    SimpleLineSymbol
      ...
      */
      //set global variable for max Extent and iniExtent    
      var iniExtent;
      ...
      return function PlaylistMap(isMobile, geometryServiceURL, bingMapsKey, webmapId, excludedLayers, dataFields, displayLegend, playlistLegendConfig, mapSelector, playlistLegendSelector, legendSelector, sidePaneSelector, onLoad, onHideLegend, onListItemRefresh, onHighlight, onRemoveHighlight, onSelect, onRemoveSelection, filterplaylistItems) {
              ...
              arcgisUtils.createMap(webmapId, mapSelector,
        {
            mapOptions:
          {
            ...
            iniExtent = new esri.geometry.Extent(
        {
            "xmin": -9122197.182774689,
            "ymin": 5053832.914335129,
            "xmax": -9063493.545051653,
            "ymax": 5102752.612437661,
            "spatialReference":
          {
              "wkid": 102100
          }
            ...
            dojo.connect(_map, "onExtentChange", function () {
                var extent = _map.extent.getCenter();
                if (iniExtent.contains(extent)) { }
                else {
                    _map.setExtent(iniExtent);
                }
            });
            ...
            */
            ...
            //Use world imary as the base map
            _map.setBasemap("hybrid");
              ...
              var basemapToggle = new BasemapToggle(
        {
                  ...
                  if (layer.featureCollection && layer.featureCollection.layers.length > 0) {
                      array.forEach(layer.featureCollection.layers, function (l) {
                          if (l.visibility && checkExcluded(l.layerObject.name)) {
                              ...
                              setRenderer(playlistLyr, layer.resourceInfo.renderer);
                      ...
                      on.once(playlistLyr, "update-end", function () {
                          ...
                          playlistLyr.queryFeatures(query).then(function (results) {
                              ...
                              setRenderer(lyr, layer.resourceInfo.drawingInfo.renderer);
                          });
          ...
          function setRenderer(lyr, renderer) {
              ...
              if (renderer) {
                  ...
                  renderer = _mapConfig.getRenderer(layerObj, lyr.graphics, colorAttr, orderAttr);
              ...
              var legend = new Legend(
        {
            map: _map
              ...
              legend.startup();
              var playlistStr = '<p class="esriLegendServiceLabel">' + playlistLegendConfig.layerTitle + '</p>';
